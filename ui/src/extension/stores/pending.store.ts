...
export const createPendingTasksStore = (initialState: PendingTasksState) => {
  ...
  const actions: PendingTasksActions = {
    ...
    exportQueue: async () => {
      return fetch('/agent-scheduler/v1/export')
        .then(response => response.json())
    },
    importQueue: async (str: string) => {
      ...
      return fetch(`/agent-scheduler/v1/import`, { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify(bodyObj) })
        .then(response => response.json())
        .then(data => {
          setTimeout(() => {
            actions.refresh();
